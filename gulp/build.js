...
gulp.task('cdn', function (callback) {
    ...
    fs.rename('dist/travis', 'dist/' + version, function (err) {
        if (err) {
            ...
            console.log('\n----++++ https://cdn.antwerpen.be/zorgbedrijf_branding_scss/' + version + '/main.min.css ++++----\n');
            callback();
        }
    });
});
...
gulp.task('readme', function() {
    ...
    return gulp.src(['README.md'])
        .pipe(replace(/zorgbedrijf_branding_scss\/[0-9]\.[0-9]\.[0-9]\/main.min.css/g, 'zorgbedrijf_branding_scss/' + version + '/main.min.css'))
        .pipe(gulp.dest('./'));
});
